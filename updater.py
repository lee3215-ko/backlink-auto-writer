...
RELEASE_ASSET = "BacklinkWriter.zip"
UPDATE_LOG_NAME = "BacklinkWriter_update.log"
...
def get_install_dir() -> Path:
    from app_paths import get_install_dir as _dir
...
def _versioned_release_url(owner: str, repo: str, version: str, asset: str) -> str:
    return f"https://github.com/{owner}/{repo}/releases/download/{_release_tag(version)}/{asset}"
...
def collect_download_urls(
    ...
    *,
    ...
    if owner_repo and version:
        ...
        urls.append(f"https://github.com/{owner}/{repo}/releases/latest/download/{asset_name}")
...
$Log = Join-Path $env:TEMP "{UPDATE_LOG_NAME}"
...
& robocopy $src $Install /E /IS /IT /XD data logs /R:8 /W:3 /NFL /NDL /NJH /NJS | Out-Null
...
def schedule_apply_update(
    ...
    *,
    ...
    staging_dir = Path(tempfile.gettempdir()) / f"BacklinkWriter_staging_{os.getpid()}"
